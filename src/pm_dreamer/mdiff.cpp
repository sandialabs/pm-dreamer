...
int main(int argc, char *argv[]) {
  ...
  while (true) {
    ...
    char cline1[token1.size()+1],cline2[token2.size()+1];
    strcpy(cline1,token1.c_str());
    strcpy(cline2,token2.c_str());
    char * tok1=strtok(cline1,"<>\"");
    while (tok1!=NULL) {
      tokens1.push_back(tok1);
      tok1=strtok(NULL,"<>\"");
    }
    const char *tok2=strtok(cline2,"<>\"");
    while (tok2!=NULL) {
      tokens2.push_back(tok2);
      tok2=strtok(NULL,"<>\"");
    }
