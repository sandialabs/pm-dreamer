...
#define Beagle_UndefinedMethodInternalExceptionM(METHOD,OBJECT,NAME) \
  Beagle::InternalException( \
    string("Method \"").append(METHOD).append("\" of class \"").append(OBJECT).append( \
...
public:
  ...
  typedef AllocatorT<InternalException,TargetedException::Alloc>
          Alloc;
  //! InternalException handle type.
  typedef PointerT<InternalException,TargetedException::Handle>
          Handle;
  //! InternalException bag type.
  typedef ContainerT<InternalException,TargetedException::Bag>
          Bag;

  explicit InternalException(string  inMessage="",
                             string  inFileName="",
