...
class ArrayT : public Object,
               public std::vector< T,BEAGLE_STLALLOCATOR<T> > {
...
Beagle::ArrayT<T>::ArrayT(unsigned int inSize) :
  std::vector< T,BEAGLE_STLALLOCATOR<T> >(inSize)
{ }
...
Beagle::ArrayT<T>::ArrayT(unsigned int inSize, const T& inModel) :
  std::vector< T,BEAGLE_STLALLOCATOR<T> >(inSize, inModel)
{ }
...
bool Beagle::ArrayT<T>::isEqual(const Beagle::Object& inRightObj) const
{
  ...
  const std::vector< T,BEAGLE_STLALLOCATOR<T> >& lLeftVector  = lLeftArray;
  const Beagle::ArrayT<T>& lRightArray = castObjectT<const Beagle::ArrayT<T>&>(inRightObj);
  const std::vector< T,BEAGLE_STLALLOCATOR<T> >& lRightVector = lRightArray;
...
void Beagle::ArrayT<T>::read(PACC::XML::ConstIterator inIter)
{
  ...
  std::vector< T,BEAGLE_STLALLOCATOR<T> >::resize(0);
  std::istringstream lISS(inIter->getValue());
  while(lISS.good()) {
    T lValue;
    lISS >> lValue;
    std::vector< T,BEAGLE_STLALLOCATOR<T> >::push_back(lValue);
...
void Beagle::ArrayT<T>::write(PACC::XML::Streamer& ioStreamer, bool inIndent) const
{
  ...
  for(unsigned int i=0; i<std::vector< T,BEAGLE_STLALLOCATOR<T> >::size(); ++i) {
    lOSS << (*this)[i];
    if(i != (std::vector< T,BEAGLE_STLALLOCATOR<T> >::size()-1)) lOSS << ',';
  }
