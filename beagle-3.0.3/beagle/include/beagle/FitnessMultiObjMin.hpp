...
public:
  ...
  typedef AllocatorT<FitnessMultiObjMin,FitnessMultiObj::Alloc>
          Alloc;
  //! FitnessMultiObjMin handle type.
  typedef PointerT<FitnessMultiObjMin,FitnessMultiObj::Handle>
          Handle;
  //! FitnessMultiObjMin bag type.
  typedef ContainerT<FitnessMultiObj,FitnessMultiObj::Bag>
          Bag;

  explicit FitnessMultiObjMin(unsigned int inSize=0, float inValue=0.0);
  explicit FitnessMultiObjMin(const std::vector< float,BEAGLE_STLALLOCATOR<float> >& inFitness);
