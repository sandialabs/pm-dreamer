...
#define Beagle_ValidateParameterM(COND,PARAM,MESS) \
  if( !(COND) ) throw Beagle::ValidationException( \
    string("The parameter ").append(PARAM).append(" is invalid (").append(MESS).append(")."))
#else  // BEAGLE_NDEBUG
#define Beagle_ValidateParameterM(COND,PARAM,MESS)
#endif // BEAGLE_NDEBUG
...
public:
  ...
  typedef AllocatorT<ValidationException,Exception::Alloc>
          Alloc;
  //! ValidationException handle type.
  typedef PointerT<ValidationException,Exception::Handle>
          Handle;
  //! ValidationException bag type.
  typedef ContainerT<ValidationException,Exception::Bag>
          Bag;

  explicit ValidationException(string inMessage="");
