 ...
 *  \ingroup GAFV
 */
class FloatVector : public Genotype,
                    public std::vector< double,BEAGLE_STLALLOCATOR<double> > {

public:

  //! FloatVector allocator type.
  typedef AllocatorT<FloatVector,Genotype::Alloc>
          Alloc;
  //! FloatVector handle type.
  typedef PointerT<FloatVector,Genotype::Handle>
          Handle;
  //! FloatVector bag type.
  typedef ContainerT<FloatVector,Genotype::Bag>
          Bag;

  explicit FloatVector(unsigned int inSize=0, float inModel=0.0);
  virtual ~FloatVector() { }
