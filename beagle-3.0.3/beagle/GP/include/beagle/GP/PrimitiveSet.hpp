...
public:
  ...
  inline GP::Primitive::Handle getPrimitiveByName(string inName) const
  {
    ...
    return lPrimitive;
    Beagle_StackTraceEndM("GP::Primitive::Handle GP::PrimitiveSet::getPrimitiveByName(string inName) const");
  }
...
protected:
  ...
  typedef std::map< unsigned int,RoulettePair,std::less<unsigned int>,
                    BEAGLE_STLALLOCATOR< std::pair<const unsigned int,RoulettePair> > >
          RouletteMap;

  std::vector< double,BEAGLE_STLALLOCATOR<double> > mSelectionBiases;  //!< Selection biases.
