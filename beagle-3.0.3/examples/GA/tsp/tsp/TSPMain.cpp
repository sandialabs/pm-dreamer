...
int main(int argc, char** argv) {
  try {
    ...
    TSPEvalOp::Handle lEvalOp = new TSPEvalOp;
    // 3. Instanciate the evolver.
    GA::EvolverIntegerVector::Handle lEvolver = new GA::EvolverIntegerVector(lEvalOp);
    // 4. Initialize the vivarium for integer vector GA population.
    GA::IntegerVector::Alloc::Handle lIVAlloc = new GA::IntegerVector::Alloc;
    FitnessSimpleMin::Alloc::Handle lFitAlloc = new FitnessSimpleMin::Alloc;
    Vivarium::Handle lVivarium = new Vivarium(lIVAlloc, lFitAlloc);
